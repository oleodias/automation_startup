...
| 5d | FALLBACK → **HTTP Request — Groq API** (ver box abaixo) → **Switch** pela intenção retornada (`confirmar`/`remarcar`/`cancelar` reaproveitam 5a-c; `duvida`/`humano` → **📤 EVOLUTION (pendente) — aviso à secretária**) | |

> **Nó de IA (5d) — Groq (plano gratuito, sem cartão):** `POST https://api.groq.com/openai/v1/chat/completions`, header `Authorization: Bearer gsk_...` (criar a chave em console.groq.com/keys) e `content-type: application/json`. Body: model `llama-3.1-8b-instant` (rápido e no free tier — 14.400 requisições/dia, muito além do que a clínica usa), `max_tokens` 20, `temperature` 0, e uma mensagem `system`: *"Classifique a mensagem de um paciente respondendo a uma confirmação de consulta. Responda APENAS uma palavra: confirmar, remarcar, cancelar, duvida ou humano."* A resposta sai em `choices[0].message.content` (formato OpenAI). Se preferirem deixar para depois, este nó também pode ser um placeholder hoje — mas dá para testar de verdade, ele não toca o WhatsApp.
>
> *Trocamos a Anthropic pela Groq porque a Groq tem free tier de verdade. A API da Groq é compatível com a da OpenAI, então trocar de provedor no futuro é só mudar URL, chave e nome do modelo — o resto do fluxo continua igual.*
